...
class BackTestSingleOrderThreadObj(threading.Thread):
...
class AAA(Process):
    def __init__(self, order_id, input_params, username, web_order_db_info, process_name, max_thread=1):
    ...
    def run_order(self, order_id, input_params):
        ...
        while try_num < max_try or break_order is False:
            ...
            self.print_c('{0}: try number: {1} wait list count: {2}'.format(self.process_name, try_num, waiting_list_count))
            ...
            while len(self.waiting_list) > 0:
                ...
                if threading.active_count() - 1 < self.max_thread:
                    ...
                    self.print_c('{0}: create thread {1} thread count: {2}'.format(self.process_name, en_symbol_12_digit_code, threading.active_count() - 1))
                    t = BackTestSingleOrderThreadObj(self.data, self.running_list, self.db_web_order, order_id)
                    # sleep(0.1)
                    ...
                    j += 1
                    if j == 1:
                        self.print_c('{0}: wait for free thread. thread count: {1}'.format(self.process_name, threading.active_count() - 1))
            ...
            while threading.active_count() > 1:
                ...
                if i > 20:
                    ...
                    if self.db_web_order.exist_order(order_id) is not True:
                        break_order = True
                        break
                self.print_c('{0}: wait to finish thread. thread count: {1}'.format(self.process_name, threading.active_count()-1))
                sleep(1)
            # sleep(1)
    ...
    def process(self):
        ...
        result, start_time, order_run_time, sum_run_time, error = self.run_order(self.order_id, self.input_params)

        if error is None:
            # insert result to db
            res, err = self.db_web_order.insert_web_order_result(order_id=self.order_id,
                                                                 username=self.username,
                                                                 input_param=str(self.input_params),
                                                                 result=str(result),
            ...
            self.print_c('{0}: finish run order: {1} : result: {2}'.format(self.process_name, self.order_id, result))

            # remove order from waiting_order table
            self.db_web_order.remove_order(self.order_id)

            # remove sub order from sub_order_result table
            if clean_sub_order_result is True:
                self.db_web_order.clean_sub_order_result(self.order_id)
...
class BackTestMultiOrderServer:
    ...
    def run_order(self, order_id, input_params, username, clean_sub_order_result=False):
        error = None
        result = True
        start_time = 0
        end_time = 0
            ...
            while threading.active_count() > 1:
                ...
                if i > 20:
                    ...
                    if self.db_web_order.exist_order(order_id) is not True:
                        break_order = True
                        break
                self.print_c('{0}: wait to finish thread. thread count: {1}'
        ...
        if error is None:
            ...
            self.print_c('{0}: finish: insert_web_order_result'.format(self.process_name))
            # res, err = self.db_analyze_data.insert_back_test_result(order_id=order_id,
            #                                                        username=username,
            #                                                        input_param=str(input_params),
            #                                                        result=str(result),
    ...
    def run_order0(self, order_id, input_params):
        error = None
        result = True
        start_time = 0
        end_time = 0
    ...
    def run(self, clean_sub_order_result=False):
        ...
        while self.get_status() != server_status_shutting_down:
            ...
            self.print_c('{0}: new order: username:{1} order id: {2}'.format(self.process_name, username, order_id))

            # -----------------------------------------------------
            # run order
            self.set_status(server_status_running)

            p = Process(target=self.run_order, args=(order_id, input_params, username, clean_sub_order_result))
            p.start()
            while self.get_status() not in [server_status_stopping,
    ...
    def run0(self, clean_sub_order_result=False):
        self.set_status(server_status_running)
            ...
            if error is None:
                ...
                for t in threading.enumerate():
                    if t != threading.main_thread():
                        ...
                        try:
                            t.join(2)
                            # signal.pthread_kill(t_id, signal.SIGKILL)
                            #os.kill(t_id, signal.SIGKILL)
